...
Namespace DataControl
    Public Class FileControl
        ...
        Function ReadFileExcel(iFileUpload As FileUpload, Optional sheetSelect As Integer = 1) As DataTable
            ...
            If (iFileUpload.HasFile AndAlso (fileExten = "xlsx" Or fileExten = "xls")) Then
                Using workBook As New XLWorkbook(iFileUpload.PostedFile.InputStream)
                    Dim workSheet As IXLWorksheet = workBook.Worksheet(sheetSelect)
                    ...
                    For Each row As IXLRow In workSheet.Rows()
                        ...
                        Else 'Add rows to DataTable.
                            tbl.Rows.Add()
                            Dim i As Integer = 0
                            'For Each cell As IXLCell In row.Cells(row.FirstCellUsed().Address.ColumnNumber, row.LastCellUsed().Address.ColumnNumber)
                            For Each cell As IXLCell In row.Cells()
                                Dim val As String = ""
                                Select Case cell.DataType
                                    ...
                                    Case Else
                                        val = cell.GetString
                                End Select
                                tbl.Rows(tbl.Rows.Count - 1)(i) = val
                                i += 1
                            Next
                        End If
                    Next
        ...
        Function ReadFileExcel(iFileUpload As FileUpload, ListSort As List(Of String),
                               Optional strSpit As String = VarIni.C8,
                               Optional sheetSelect As Integer = 1) As DataTable
            'ListSort format item = seq of column & "strSpit" & (ASC|'',DESC)
            Dim tbl As DataTable = ReadFileExcel(iFileUpload, sheetSelect)
            Dim dtViewExcleRead As New DataView(tbl)
            If ListSort.Count > 0 AndAlso tbl.Rows.Count > 0 Then
                Dim outcont As New OutputControl
                Dim orderby As String = ""
                For Each str As String In ListSort
                    Dim seq As Integer
                    Dim desc As String = ""
                    Dim val() As String = str.Split(strSpit)
                    Dim line As Integer = 0
                    For Each txt As String In val
                        Select Case line
                            Case 0
                                seq = outcont.checkNumberic(txt)
                            Case 1
                                desc = " ASC "
                                If txt = "DESC" Then
                                    desc = " DESC "
                        ...
                        line += 1
                    Next
                    orderby &= tbl.Columns(seq).ColumnName & desc & VarIni.C
                Next
                dtViewExcleRead.Sort = " " & orderby.Substring(0, orderby.Length - 1) & " "
            End If
            Return dtViewExcleRead.ToTable
        End Function
