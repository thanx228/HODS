...
Public Class EditTransferOrder
    ...
    Private Sub GridView2_RowCommand(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewCommandEventArgs) Handles GridView2.RowCommand
        ...
        If e.CommandName = "OnChange" Then
            ...
            Dim dSeq As String = GridView2.Rows(i).Cells(2).Text.Replace("&nbsp;", "")
            Ltype.Text = dType
            Lno.Text = dNo
            Lseq.Text = dSeq
            ...
            End If
            Dim sSQL As String = "select top 1 MA005 from MOCMA where MA012 <= TB003 and MA001=TC047 and (MA002=TC007 or MA002=TC009 ) and (MA003=TB005 or MA003=TB008) order by MA012 desc "
            sql = " select isnull((" & sSQL & "),0) as MA005 from SFCTC left join SFCTB on TB001=TC001 and TB002=TC002 " & _
                  " where TC001='" & dType & "' and TC002='" & dNo & "' and TC003='" & dSeq & "' "
            aa = New Data.DataTable
            aa = Conn_sql.Get_DataReader(sql, Conn_sql.ERP_ConnectionString)
            If aa.Rows.Count > 0 Then
                Lprc.Text = aa.Rows(0).Item("MA005").ToString()
                txtprice.Text = aa.Rows(0).Item("MA005").ToString()
            End If
            GridView2.Visible = True
        End If
    ...
    Private Sub BuSave_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles BuSave.Click
        ...
        If CDec(Lprc.Text.Trim) = 0 Then
            show_message.ShowMessage(Page, "Price is zero", UpdatePanel1)
            GridView2.Visible = True
            Exit Sub
        End If
