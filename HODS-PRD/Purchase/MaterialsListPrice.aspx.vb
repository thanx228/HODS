Public Class MaterialsListPrice
    ...
    Protected Sub btShow_Click(sender As Object, e As EventArgs) Handles btShow.Click
        ...
        Select Case ddlSearchBy.Text
            ...
            Case "2" 'Sale Order
                ...
                If saleSeq <> "" Then
                    WHR = " and TD003 like'%" & saleSeq & "' "
                End If
                SQL = "select TD004 as parentItem,sum(TD008) as orderQty from COPTD where TD001='" & saleType & "' and TD002='" & saleNo & "' " & WHR & " group by TD004"
            Case "3" 'Manufactor Order
                ...
                SQL = "select TA006 as parentItem,TA015 as orderQty from MOCTA where TA001='" & workType & "' and TA002='" & workNo & "' "
        End Select
        ...
        'stock        
        SQL = " select INVML.ML001 as item,sum(INVML.ML005) as supportQty from DBMIS.dbo." & tempTable2 & " T " & _
              ...
              " where INVML.ML002 in ('2201','2400','2900','3000','3333') group by INVML.ML001 "
        Program = Conn_SQL.Get_DataReader(SQL, Conn_SQL.ERP_ConnectionString)
        For i As Integer = 0 To Program.Rows.Count - 1
            item = Program.Rows(i).Item("item")
        ...
        SQL = " select MatItem from " & tempTable & " where Property<>'M' group by MatItem "
        Program = Conn_SQL.Get_DataReader(SQL, Conn_SQL.MIS_ConnectionString)
        For i As Integer = 0 To Program.Rows.Count - 1
            fldInsHash = New Hashtable
            whrHash = New Hashtable
            With Program.Rows(i)
                Dim supplier As String = "", price As Decimal = 0,
                    exchangRate As Decimal = 1, currency As String = ""
                code = .Item("MatItem").ToString.Trim
                sSQL = "select top 1 MB002 as supplier,MB003 as currency,MB011 as price from PURMB where MB001='" & code & "' order by MB014 desc,MB011 "
                sProgram = Conn_SQL.Get_DataReader(sSQL, Conn_SQL.ERP_ConnectionString)
                If sProgram.Rows.Count > 0 Then
                    With sProgram.Rows(0)
                        supplier = .Item("supplier")
                        price = .Item("price")
                        currency = .Item("currency").ToString.Trim
                        If currency <> "THB" Then
                            Dim yearMonth As String = DateTime.Today.ToString("yyyyMM")
                            ssSQL = " select top 1 MG004 as exchangRate from CMSMG where MG001='" & currency & "' and MG002 like '" & yearMonth & "%' and MG002<'" & yearMonth & "06' and MG004>0 order by MG002 desc"
                            ssProgram = Conn_SQL.Get_DataReader(ssSQL, Conn_SQL.ERP_ConnectionString)
                            If ssProgram.Rows.Count > 0 Then
                                exchangRate = ssProgram.Rows(0).Item("exchangRate")
                            End If
                        End If
                    End With
                    fldInsHash = New Hashtable
                    ...
                    whrHash = New Hashtable
                    'whr of condition
                    whrHash.Add("MatItem", code) ' Sub item
                    'Update Zone
                    ...
                    Conn_SQL.Exec_Sql(Conn_SQL.GetSQL(tempTable, fldUpdHash, whrHash, "U"), Conn_SQL.MIS_ConnectionString)
                End If
            End With
        Next
        ...
        SQL = "select T1.DocNo as 'Doc No',T1.ParentItem as 'Parent Item', " & _
            ...
            " Unit,Currency,Price as 'Price Original',ExchangRate as 'Exchang Rate'," & _
            " cast(Price*ExchangRate as decimal(10,4)) as 'Price THB', " & _
            " Supplier,PURMA.MA002 as 'Supplier Description'," & _
            " cast(T1.orderQty*T1.QtyPerPcs*Price*ExchangRate as decimal(10,4)) as 'Amount THB', " & _
            ...
            " left join JINPAO80.dbo.PURMA PURMA on PURMA.MA001= T1.Supplier " & _
        ...
        For i As Integer = 0 To Program.Rows.Count - 1
            With Program.Rows(i)
                If .Item("MB025") = "M" Then
                    CodeBOM(tempTable, docNo, parentItem, .Item("MD003"), .Item("MD006"), orderQty)
                Else
                    Dim fldInsHash As Hashtable = New Hashtable,
                        whrHash As Hashtable = New Hashtable,
    ...
    Private Sub gvShow_RowDataBound(sender As Object, e As System.Web.UI.WebControls.GridViewRowEventArgs) Handles gvShow.RowDataBound
        If e.Row.RowType = DataControlRowType.DataRow Then
            ...
            If Not IsNothing(hplDetail) And Not IsDBNull(e.Row.DataItem("Sub Item")) Then
                ...
                hplDetail.NavigateUrl = "../Planing/MaterialsListPopup.aspx?height=150&width=350" & link
