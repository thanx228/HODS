...
Public Class MachineTable
    ...
    Dim Machine1CTime() As String = {"MachineC01", "MachineC02", "MachineC03", "MachineC04", "MachineC05", "MachineC06", "MachineC07", "MachineC08", "MachineC09", "MachineC10", "MachineC11", "MachineC12", "MachineC13", "MachineC14", "MachineC15", "MachineC16", "MachineC17", "MachineC18", "MachineC19"}
    Dim Machine1DTime() As String = {"MachineD01", "MachineD02", "MachineD03", "MachineD04", "MachineD05", "MachineD06", "MachineD07", "MachineD08", "MachineD09", "MachineD10", "MachineD11", "MachineD12", "MachineD13", "MachineD14", "MachineD15", "MachineD16", "MachineD17", "MachineD18", "MachineD19"}
        ...
        For i = 1 To 8
            ...
            For j = 0 To 18
                ...
                If Not NowShowTimeButton Is Nothing Then
                    ...
                    Dim strSql1 As String = "select MX003,MX006 from CMSMX where MX001='" & Right(ThisMachine(j), 3) & "'"
                    ...
                    'End Select
                    NowShowTimeButton.ForeColor = Drawing.Color.Black
                    Dim SQL As String = "select NowStatus,PlanFinishTime,MONo from NowMachine where NowDate='" & SelectDate & "' and MachineID='" & Right(ThisMachine(j), 3) & "'"
                    Dim dt2 As DataTable = dbConn.Query(SQL, VarIni.DBMIS, dbConn.WhoCalledMe)
                    ...
                    Else
                        ...
                        If Not IsDBNull(dt2.Rows(0).Item(2)) Then
                            ...
                            TooltipShow = dt5.Rows(0).Item(1) & "-" & Right(ThisMachine(j), 3) & "&#13;" & Trim(dt2.Rows(0).Item("MONo")) & "&#13;" & ItemSpec & "&#13;Pre:" & PrePorcessName & "&#13;Next:" & NextPorcessName
                            End If
                        ...
                        If dtLastMachine.Rows.Count > 0 Then
                            If Trim(dtLastMachine.Rows(0).Item("LastMachineID")) = Right(ThisMachine(j), 3) Then
    ...
    Protected Sub MachineSelect(sender As Object, e As EventArgs) 'Handles MachineA01.Click
        LBNowSelectMachine.Text = Right(sender.ID, 3)
            ...
            Case Else
                ...
                If ThisMachineStatus = 4 Then
                    ...
                    For EmpCount = 0 To dtHR.Rows.Count - 1
                        Dim SQLcheck As String = "select count(*) from NowMachine where EmpCode like '%" & Trim(dtHR.Rows(EmpCount).Item(0)) & "%' and NowDate='" & SelectDate & "' and NowStatus=5"
                ...
                If Not IsDBNull(dtSMOWorkRecord2.Rows(0).Item(0)) Then
                    LBProcFinishQty.Text = dtSMOWorkRecord2.Rows(0).Item(0)
                Else
    ...
    "where MOC.TA001='" & MOGroup(0) & "' and MOC.TA002='" & MOGroup(1) & "' and SFC.TA003='" & MOGroup(2) & "'"
                ...
                If dtSQLPlanSchedule.Rows.Count > 0 Then
                    LBProcessQty.Text = dtSQLPlanSchedule.Rows(0).Item("PlanQty")
                Else
                    LBProcessQty.Text = "0"
                End If
                ...
                LBMOQty.Text = dtData.Rows(0).Item("TA015")
                ...
                If dtSMOWorkRecord1.Rows.Count > 0 Then
                    If LBRID.Text = "" Then
                        LBRID.Text = dtSMOWorkRecord1.Rows(0).Item("RID")
                        If Not IsDBNull(dtSMOWorkRecord1.Rows(0).Item("Note")) Then
                            TBNote.Text = dtSMOWorkRecord1.Rows(0).Item("Note")
                        Else
                            TBNote.Text = ""
                        End If
                    End If
...
"order by NowDate desc,EndTime desc"
    ...
    "where MOC.TA001='" & MOGroup(0) & "' and MOC.TA002='" & MOGroup(1) & "' and SFC.TA003='" & MOGroup(2) & "'"
                    ...
                    LBHisMOQty.Text = dtHis1.Rows(0).Item("TA015")
                    LBHisPlanQty.Text = ""
                    LBHisFinishQty.Text = dtHis.Rows(0).Item("Qty")
        ...
        Select Case DDLMachineAllStatus.SelectedValue
            Case "1"          'Idle
                ...
                If NowSelectMachineStatus = "2" Or NowSelectMachineStatus = "3" Or NowSelectMachineStatus = "4" Then
                    ...
                    If NowSelectMachineStatus = "3" Then
                        ...
                        strSql = "update MOWorkRecord set EndTime='" & EndTime & "',EndDate='" & SelectDate & "',Qty='" & TBFinishQty.Text & "',Note=N'" & TBNote.Text & "' where MONo='" & MoNo & "' and NowDate='" & SelectDate & "' and MachineID='" & LBNowSelectMachine.Text & "' and EndTime is null and WorkType=1"
                        Conn_SQL.Exec_Sql(strSql, Conn_SQL.MIS_ConnectionString)
                    End If
                ...
                If NowSelectMachineStatus = "5" And TBFinishQty.Text <> "" Then
                    ...
                    Dim EndTime As String = Hour(Now).ToString.PadLeft(2, "0") & Minute(Now).ToString.PadLeft(2, "0")
                    strSql = "update MOWorkRecord set EndTime='" & EndTime & "',EndDate='" & SelectDate & "',Qty='" & TBFinishQty.Text & "',Note=N'" & TBNote.Text & "' where MONo='" & MoNo & "' and NowDate='" & SelectDate & "' and MachineID='" & LBNowSelectMachine.Text & "' and EndTime is null and WorkType=2"
                    Conn_SQL.Exec_Sql(strSql, Conn_SQL.MIS_ConnectionString)
            ...
            Case "2"          'Wait Setup Mold
                ...
                Else
                    If NowSelectMachineStatus = "2" Then
                        Dim strSql As String
                        strSql = "update MOWorkRecord set Note=N'" & TBNote.Text & "' where RID='" & LBRID.Text & "'"
                        Conn_SQL.Exec_Sql(strSql, Conn_SQL.MIS_ConnectionString)
                    End If
                End If
            ...
            Case "4"
                ...
                Else
                    ...
                    Dim MoGroup() As String = MoNo.Split("-")
                    strSql = "update PlanSchedule set SetupMold=1 where PlanDate='" & SelectDate & "' and MoType='" & MoGroup(0) & "' and MoNo='" & MoGroup(1) & "' and MoSeq='" & MoGroup(2) & "'"
                    Conn_SQL.Exec_Sql(strSql, Conn_SQL.MIS_ConnectionString)
